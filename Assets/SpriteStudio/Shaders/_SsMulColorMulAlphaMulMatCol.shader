Shader "Ss/MulColorMulAlphaMulMatCol"
{
	Properties
	{
	...
	Category
	{
		...
		SubShader {
			Pass {
				...
				struct v2f {
					...
					float2	uv				: TEXCOORD;
					fixed4	color			: COLOR0;
					fixed4	colorBlendRate	: TEXCOORD1;	// values from texcoord1 field.
				};
				...
				v2f vert(appdata_ss v)
				{
					...
					// pass extra info
					o.colorBlendRate = v.color;	// color blend rate
		...
		SubShader {
			Pass {
				SetTexture [_MainTex] {ConstantColor [_OneColor]
